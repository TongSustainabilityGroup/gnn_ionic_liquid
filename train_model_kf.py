...
class ILDataModule(pl.LightningDataModule):
    def __init__(self, df, batch_size, fold):
        super().__init__()
        ...
        kf_idx_list = [] 
        for fold, (train_ids, test_ids) in enumerate(kf.split(np.arange(len(self.df)))):
            kf_idx_list.append([train_ids, test_ids])

        self.df0 = self.df.iloc[kf_idx_list[fold][0]]
        self.df1 = self.df.iloc[kf_idx_list[fold][1]]
        self.stat_dict = {
            'inp_mean': self.df0[['catalyst_amount', 'solvent_amount', 'temperature_c', 'reaction_time_min','PET_source','PET_size_mm']].mean(),
            'inp_std': self.df0[['catalyst_amount', 'solvent_amount', 'temperature_c', 'reaction_time_min','PET_source','PET_size_mm']].std(),
            'out_mean': self.df0['target'].mean(), 
            'out_std': self.df0['target'].std()
        }

        self.df0[['catalyst_amount', 'solvent_amount', 'temperature_c', 'reaction_time_min','PET_source','PET_size_mm']] = (
            (self.df0[['catalyst_amount', 'solvent_amount', 'temperature_c', 'reaction_time_min','PET_source','PET_size_mm']]
            - self.stat_dict['inp_mean']) / self.stat_dict['inp_std']
        )
        
        self.df0['target'] = (self.df0['target'] - self.stat_dict['out_mean']) / self.stat_dict['out_std']
        
        self.df1[['catalyst_amount', 'solvent_amount', 'temperature_c', 'reaction_time_min','PET_source','PET_size_mm']] = (
            (self.df1[['catalyst_amount', 'solvent_amount', 'temperature_c', 'reaction_time_min','PET_source','PET_size_mm']]
            - self.stat_dict['inp_mean']) / self.stat_dict['inp_std']
        )
        self.df1['target'] = (self.df1['target'] - self.stat_dict['out_mean']) / self.stat_dict['out_std']
    ...
    def setup(self, stage=None):
        if (stage == 'fit' or stage is None):
            self.df0['data'] = self.df0.apply(self.prep, axis=1)
            self.train_data_list = self.df0['data'].tolist()

            self.df1['data'] = self.df1.apply(self.prep, axis=1)
            self.test_data_list = self.df1['data'].tolist()

    def train_dataloader(self):
        return torch_geometric.loader.DataLoader(self.train_data_list, batch_size=self.batch_size, shuffle=True, drop_last=True)

    def val_dataloader(self):
        return torch_geometric.loader.DataLoader(self.test_data_list, batch_size=len(self.test_data_list), shuffle=False, drop_last=False)  
...
class ILPredictor(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/lossl2", slossl2, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/lossl1", slossl1, on_step=False, on_epoch=True, prog_bar=True)
...
for fold in range(4):
    ...
    ILdata = ILDataModule(df2, batch_size=8, fold=fold)
    ILdata.setup()

    trainer.fit(gmodel, ILdata.train_dataloader(), ILdata.val_dataloader())
    trainer.save_checkpoint(f"kf{fold}.ckpt")
